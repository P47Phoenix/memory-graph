...
//! `open_store`, index files by extension, and query its symbols on both
//! backends.
use graph_store::{open_store, Backend, SymbolQuery};
use toy_extractor::IniExtractor;

#[test]
fn registered_extractor_is_indexed_and_queryable() {
    for backend in [Backend::Redb, Backend::RedbV2] {
        let dir = tempfile::tempdir().unwrap();
        let store = open_store(
            backend,
            &dir.path().join("g.redb"),
            vec![Box::new(IniExtractor)],
        )
        .unwrap();
        store
            .index_bytes(
        ...
        assert_eq!(hits[0].qualified, "server::port", "{backend:?}");
        let info = store.describe(None, None).unwrap();
        let kinds = &info[0].languages["ini"].symbol_kinds;
        assert_eq!(kinds.get("module/section"), Some(&1), "{backend:?}");
        assert_eq!(kinds.get("variable/key"), Some(&2), "{backend:?}");
    }
}
