...
## How to run
It needs the `graph-store` crate as of commit **`293bb2a`** (main before PR #8; the spike calls `Store::index_batch` and `BatchFile`, which exist there; it may not compile against later main without edits) and two dev-dependencies that are deliberately not in the workspace.
