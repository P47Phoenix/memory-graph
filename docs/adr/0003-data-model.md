...
7. What it costs: about 45-47 developer-days for the core, plus 27 days for the "many database files" (sharding) design, and about 78-80 days with everything (give or take 30%). The risks are that token ids stop being stable, the file format becomes a long-term commitment, and the numbers come from a prototype.
8. Nothing is decided yet. The status is Proposed. A checkpoint (ADR story 4) will say go or no-go using real numbers.
9. The user has already made four decisions. D1: it must handle more than 100 million tokens and spread across several database files. D2: before version 1.0 we may simply re-index, but from 1.0 on old data must convert safely. D3: a reader sees a frozen, consistent picture while a writer works, like a photo of the shelves. D4: the experiment code stays in the repo, outside the main build.
10. Two questions block later work. Q4: how big should one database file get, and how do we decide which repos go in which file (like deciding when a shelf is full and which books go on the next one)? Q5: only one program at a time can open the database file, so a second program (the command line) gets locked out while the writer runs, like a single-key reading room. We must pick a fix.
...
| Q4 (**blocks** stories 14-17) | How big is one database file, and how are repos assigned? | Sets how the design splits when it grows. | Choose the unit and the key. | One file is a shard; key is (org, repo); one shard by default; split at a set size (e.g. 200 M tokens or 20 GB). |
| Q5 (**blocks** story 10) | How does the command line read while another program holds the file? | The file lock stops other programs opening it, so D3 is not met across programs. | (a) one program owns the file and serves everyone; (b) the command line retries and waits; (c) several files so writers leave the read files free. | None. The user must choose. |
...
The technical text starts below. Everything above is a summary of it. Nothing in the technical text was changed in meaning.
...
- **Constraint (Q5, VERIFIED in redb 2.6.3):** `src/tree_store/page_store/file_backend/unix.rs` (~L37-41) takes `flock(LOCK_EX | LOCK_NB)` on open; if another handle holds it, open fails **immediately** with `DatabaseAlreadyOpen`. 
  ...
  - **D3 is NOT satisfied for today's CLI-vs-writer topology** (CLI and MCP/indexer are separate processes on one file). This is a **blocking user decision (Q5)**: options are (a) a single owning daemon/MCP process that serves all reads and writes (CLI talks to it); (b) CLI retry-with-timeout/back-off on `DatabaseAlreadyOpen` (no snapshot guarantee across processes, only availability between writes); (c) per-shard files, with writers on other shards leaving the queried shards openable. 
  ...
  - **What this means (Q5):** only one program may hold the database file at a time, and others are turned away at once. Frozen views therefore protect readers only inside the owning program. We need the user to choose how the command line reads.
