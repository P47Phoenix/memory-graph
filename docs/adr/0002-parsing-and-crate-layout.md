...
**TL;DR:** three crates (core, store, cli); extractors return spans, the store derives hierarchy; fallback tokenizer for any language; pure-Rust gate in CI.
...
- Any language works through the fallback tokenizer. Rich extractors (Rust first, story 9) and NDJSON ingest (epic story 13) add symbols.
