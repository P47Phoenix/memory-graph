...
**Consequences:** schema version is stored in a `meta` table and checked before any write (currently 2; version 1 databases are upgraded in place, older builds refuse version 2). Re-indexing a file replaces its subtree. Open: JSON node encoding is large (see spike); `fjall` (LSM) comparison is pending.
