...
The symbol span from `syn` is right (it ends at 25); the tokenizer ends the string token early at the inner `"` and produces a token that straddles the symbol boundary. The store's rejection is valid. The remaining problem is that one bad file aborts the whole batch (nothing from that batch is stored), which is what aborted the first long-index attempt; the crates were removed from the test input. Both the tokenizer fix (Rust raw and byte strings, in a Rust-only tokenizer dialect) and per-file `InvalidSpan` isolation were outside this spike's scope and landed afterwards (issue #16). Logs: `logs/s2_lock.txt` (first run, section 3 shows the failure as an early exit at 8.5 s) and `logs/s2_lock_index_rerun.txt`.
