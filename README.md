...
**Per-file failures.** If a file's extraction fails span validation (`invalid span`, an extractor/tokenizer bug), only that file fails: it is not stored (any previously stored version is left as is), the other files in the batch are stored, `index` prints `failed: <path>: <reason>` (and `failed`/`failed_files` in `--json`), skips `--prune` with a warning, and exits non-zero after finishing. Storage errors still abort the whole batch. The generic tokenizer handles Rust raw strings (`r#".."#`, `br".."`) and byte strings/chars (`b".."`, `b'x'`) as literals (tokenizer version 2).
