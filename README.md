...
**Parallel parsing and progress.** `index` walks the directory first, then reads and parses files on `--jobs N` threads (`-j`; default one per CPU) while a single writer commits them in walk order and in the same batches whatever `N` is, so the database file and the summary are identical for any `--jobs` (the e2e test checks this byte for byte on both backends). Memory stays bounded: only a small window of parsed files waits for the writer. While it runs, a live display on stderr shows an overall bar (files, MB, rate, ETA, skipped and failed counts) and one line per worker with the file it is parsing (at most 16 lines, then `… +N more workers`). It is drawn only when stderr is a terminal: by default when `--json` is off, with `--progress` even with `--json`, never with `--no-progress`. A panicking extractor stops the run with an error naming the file (whole committed batches stay stored). stdout carries only the final summary either way. Library note: the split behind this is `Store::prepare` (pure, callable from many threads) and `Store::index_prepared` (the commit); see ADR 0003.
